        ...
        <div className="p-8 max-w-6xl mx-auto animate-in fade-in duration-500">
            <div className="flex justify-between items-center mb-8">
                <div>
                    <h1 className="text-2xl font-serif text-white flex items-center gap-3">
                        <BookOpenText className="text-[#B8B8B8]" /> Clause Library
                    </h1>
                    <p className="text-sm text-zinc-400 mt-1">Manage your company's approved clauses for AI drafting.</p>
                </div>
                {!isAdding && (
                    <button 
                        onClick={() => setIsAdding(true)}
                        className="bg-[#B8B8B8] text-[#0A0A0A] px-4 py-2 rounded-lg font-bold text-xs uppercase tracking-wider hover:bg-[#D4D4D4] transition-all shadow-[0_0_15px_rgba(184, 184, 184,0.2)] hover:shadow-[0_0_20px_rgba(184, 184, 184,0.4)]"
                    >
            ...
            {isAdding && (
                <div className="bg-[#141414] border border-white/10 rounded-xl p-6 mb-8 relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-[#B8B8B8] to-transparent"></div>
                    
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-lg font-serif text-white flex items-center gap-2">
                            <Plus className="text-[#B8B8B8] w-5 h-5" />
                            New Standard Clause
                        </h2>
                        <button 
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input 
                                    ...
                                    className="w-full bg-[#0a0a0a] border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white focus:border-[#B8B8B8] focus:ring-1 focus:ring-[#B8B8B8] outline-none transition-all placeholder:text-zinc-600"
                                    required
                                />
                            </div>
                            <div>
                            ...
                            <div className="flex gap-4">
                                <label className={`flex-1 flex items-center justify-center gap-2 p-3 rounded-lg border cursor-pointer transition-all ${
                                    formData.clause_type === 'Standard' 
                                        ? 'bg-[#B8B8B8]/10 border-[#B8B8B8] text-[#B8B8B8]'
                                ...
                                <label className={`flex-1 flex items-center justify-center gap-2 p-3 rounded-lg border cursor-pointer transition-all ${
                                    formData.clause_type === 'Fallback' 
                                        ? 'bg-[#1C1C1C] border-[#3A3A3A] text-[#D4D4D4]'
                                        : 'bg-[#0a0a0a] border-white/10 text-zinc-400 hover:border-white/30'
                                }`}>
                                    <input 
                                        type="radio" 
                        ...
                        <div>
                            ...
                            <textarea 
                                ...
                                className="w-full h-40 bg-[#0a0a0a] border border-white/10 rounded-lg px-4 py-3 text-sm text-white font-serif focus:border-[#B8B8B8] focus:ring-1 focus:ring-[#B8B8B8] outline-none transition-all placeholder:text-zinc-600 resize-y"
                                required
                            />
                        </div>
                        ...
                        <div className="flex justify-end pt-2">
                            <button 
                                ...
                                className="bg-[#B8B8B8] text-[#0A0A0A] px-6 py-2.5 rounded-lg font-bold text-xs uppercase tracking-wider hover:bg-[#D4D4D4] transition-all flex items-center justify-center gap-2 min-w-[160px] disabled:opacity-50"
                            >
                                {isSubmitting ? (
                                    <>
                                ...
                                {isLoading ? (
                                    <tr>
                                        <td colSpan={4} className="p-8 text-center text-zinc-500">
                                            <div className="w-6 h-6 border-2 border-[#B8B8B8] border-t-transparent rounded-full animate-spin mx-auto mb-3"></div>
                                    ...
                                    clauses.map((clause) => (
                                        <tr key={clause.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors group">
                                            <td className="p-4 align-top">
                                                <div className="font-semibold text-white text-sm group-hover:text-[#B8B8B8] transition-colors">
                                                    {clause.title}
                                                </div>
                                            </td>
                                            <td className="p-4 align-top">
                                            ...
                                            <td className="p-4 align-top">
                                                <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded border ${
                                                    clause.clause_type === 'Standard' 
                                                        ? 'bg-[#B8B8B8]/10 text-[#B8B8B8] border-[#B8B8B8]/30'
                                                        : 'bg-zinc-800/70 text-zinc-300 border-zinc-700'
                                                }`}>
                                                    {clause.clause_type}
                                                </span>
                                            </td>
